...
public class Reform extends DForm {
    ...
    public ActionResult behave(NCAtTGlyph glyph) {         
        ...
        for (int i = 0; i < tessData.vertices.length; i++) {
        	offset[0] = origTessData.vertices[i][0] - tessData.vertices[i][0];
        	offset[1] = origTessData.vertices[i][1] - tessData.vertices[i][1];
            ...
            tessData.vertices[i][0] += offset[0];
            tessData.vertices[i][1] += offset[1];
		}
        
        tessDataProp.set(tessData);

        if (done) {
        	glyph.setDeformed(false);
